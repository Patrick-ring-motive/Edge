...
const Button = ({
  ...
  return ( <
    ...
    className = {
      `${active && 'font-bold'} cursor-pointer mr-3 p-1`
    }
    />
  )
}
...
export default function Post() {
  ...
  const renderElement = useCallback((props) => < Element {
      ...
      const renderLeaf = useCallback((props) => < Leaf {
          ...
          const [currentMark, setCurrentMark] = useState(null) const [publishing, setPublishing] = useState(false) const [disabled, setDisabled] = useState(true) const [savedState, setSavedState] = useState('Saving post') const [postData, setPostData] = useState() // The created post.
