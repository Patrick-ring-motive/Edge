...
const serialize = (node) => {
  if (Text.isText(node)) {
    if (node.code) {
      return < Highlight className = "h-full" > {
        node.text
      } < /Highlight>
    }
    if (node['heading-one']) {
    ...
    if (node.bold && node.italic) {
      return < p className = "font-bold italic font-cal" > {
        node.text
      } < /p>
    }

    if (node.bold) {
      return < p className = "font-bold font-cal" > {
        node.text
      } < /p>
    }

    if (node.italic) {
      return < p className = "font-italic font-cal" > {
        node.text
      } < /p>
    }

    if (node['heading-two']) {
      return < p className = "text-2xl font-cal" > {
        node.text
      } < /p>
    }
  ...
  switch (node.type) {
    ...
    case 'code':
      return < code className = "bg-gray-50 p-2 m-2" > {
        children
      } < /code>
